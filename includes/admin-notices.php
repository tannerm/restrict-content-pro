...
function rcp_upgrade_needed_notice() {
	if( rcp_check_if_upgrade_needed() ) {
		echo '<div class="error"><p>' . __( 'The Restrict Content Pro database needs updated: ', 'rcp' ) . ' ' . '<a href="' . dd_query_arg( 'rcp-action', 'upgrade', admin_url() ) . '">' . __( 'upgrade now', 'rcp' ) . '</a></p></div>';
	}
